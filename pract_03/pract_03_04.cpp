...
class ResidueClass31 { // класс классов вычетов по модулю 31
    ...
    public:
        ...
        bool operator== (const ResidueClass31& a) { // перегрузка оператора проверки на равенство
            return (x == a.x);
        }
        bool operator!= (const ResidueClass31& a) { // перегрузка оператора проверки на неравенство
            return (x != a.x);
        }
        bool isQuadratic() { // метод проверки на квадратичность
            for (int i = 0; i < n; i++) {
                if ((i * i) % n == x) {
                    return true;
                }
...
int main() { // главная функция
    ...
    int c;
    cin >> a >> b >> c;
    cout << (a + b) << " " << (a - b) << " " << (a * b) << " " << (a ^ c) << " " << a.inverse() << " " << (a / b) << " " << a.isQuadratic() << " " << (a == b) << " " << ((a + a) != b) << endl;
}